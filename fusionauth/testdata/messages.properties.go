...
#
{tooltip}remember-device=Check this to stay signed into %[1]s for the configured duration, do not select this on a public computer or when this device is shared with multiple users
...
[EmailVerificationDisabled]=Email verification functionality is currently disabled. Contact your %[1]s administrator for assistance.
...
[InvalidEmail]=%[1]s was unable to find a user with that email address.
...
[MissingPKCECodeVerifier]=The code_verifier could not be determined, this request likely did not originate from %[1]s. Unable to complete this login request.
...
[SSOSessionDeletedOrExpired]=You have been logged out of %[1]s.
[TenantIdRequired]=%[1]s is unable to determine which tenant to use for this request. Please add the tenantId to the URL as a request parameter.
...
# Self Service
[SelfServiceFormNotConfigured]=Configuration is incomplete. The %[1]s administrator must configure a form for this application.
