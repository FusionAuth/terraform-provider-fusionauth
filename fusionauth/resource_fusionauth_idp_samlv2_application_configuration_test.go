...
func TestAccIdpSamlV2ApplicationConfiguration(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		PreCheck:          func() { testAccPreCheck(t); skipIfFusionAuthBelow(t, "1.65.0") },
		...
		Steps: []resource.TestStep{
			...
			// Verify importing the resource round-trips all fields.
			{
				ResourceName:      "fusionauth_idp_saml_v2_application_configuration.test_0",
				ImportState:       true,
	...
	// Field defaults and override clearing: an omitted enabled must default to false, and a
	// removed button_text must actually be cleared on the server — the merge patch has to send
	// an explicit null, since an omitted key would leave the old value in place.
	resource.Test(t, resource.TestCase{
		PreCheck:          func() { testAccPreCheck(t); skipIfFusionAuthBelow(t, "1.65.0") },
		ProviderFactories: testAccProviderFactories,
		Steps: []resource.TestStep{
	...
	// Explicit ownership handoff: setting manage_application_configurations = false while
	// removing the inline block must leave the entries untouched on the server (so they can be
	// imported into standalone resources), and combining false with an inline block must be
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Hand off: drop the inline block and disable management. The entry must survive.
			{
				Config: testAccIDPSAMLv2ApplicationConfigurationConfig(t,
					&testAccIdpSamlV2ApplicationConfigurationResourceTemplateConfig{
						ResourceName:       resourceName,
			...
			// Combining false with an inline block is rejected at plan time.
			{
				Config: testAccIDPSAMLv2ApplicationConfigurationConfig(t,
					&testAccIdpSamlV2ApplicationConfigurationResourceTemplateConfig{
						ResourceName:       resourceName,
				...
				ExpectError: regexp.MustCompile("application_configuration cannot be set when manage_application_configurations is false"),
			},
		},
	})
	...
	//
	// Multiple standalone configurations rely on the PATCH race condition fix in FusionAuth 1.65.0.
	resource.Test(t, resource.TestCase{
		PreCheck:          func() { testAccPreCheck(t); skipIfFusionAuthBelow(t, "1.65.0") },
		ProviderFactories: testAccProviderFactories,
		Steps: []resource.TestStep{
...
type testAccIdpSamlV2ApplicationConfigurationResourceTemplateConfig struct {
	...
	StandaloneApp0Minimal bool
	UnmanagedAppConfig    bool
