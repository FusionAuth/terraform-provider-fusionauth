...
~> **You must set `manage_application_configurations = false` on the `fusionauth_idp_saml_v2` resource** when using this resource. Otherwise any update to the identity provider resource removes the configurations managed here. Setting it to `false` also rejects accidental in-line blocks at plan time, and enables a zero-downtime migration from in-line management: set the flag to `false`, remove the in-line blocks (the entries are left untouched), then `terraform import` them into this resource.

-> This resource updates the identity provider with a partial (`PATCH`) request. FusionAuth `1.65.0` or later is required for safe concurrent use — earlier versions contain a race condition in the PATCH method that can lose updates when multiple configurations for the same identity provider are applied in parallel.
